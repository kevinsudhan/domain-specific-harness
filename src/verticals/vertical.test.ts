...
import { ALL_VERTICALS } from "./all.js";
...
console.log("\n4b. Every shipped vertical is well-formed, not just the active one");
{
  // The whole claim of the harness is that the kernel is domain-agnostic. That is only
  // worth asserting if every vertical is held to the same validator as freight — an
  // unreachable state or an undeclared action in a template nobody is running today is a
  // trap for whoever switches to it.
  check("eleven templates ship", ALL_VERTICALS.length === 11, String(ALL_VERTICALS.length));

  for (const v of ALL_VERTICALS) {
    const problems = validateVertical(v);
    check(`${v.id}: config valid`, problems.length === 0, problems);

    const t = byId(v.id);
    check(`${v.id}: in the template registry`, Boolean(t));
    check(`${v.id}: registry lifecycle matches the config`,
      t?.lifecycle.join() === [...v.lifecycle.order].join());
    check(`${v.id}: registry always-approve matches the config`,
      [...(t?.alwaysApprove ?? [])].sort().join() === Object.keys(v.policy.alwaysApprove).sort().join());
    check(`${v.id}: routable`, t?.ready === true);
    check(`${v.id}: has routing vocabulary`, (t?.vocabulary.length ?? 0) >= 5, t?.vocabulary.length);
  }

  const ids = ALL_VERTICALS.map((v) => v.id);
  check("no duplicate ids", new Set(ids).size === ids.length, ids);
}
