...
import { pick, byId, TEMPLATES } from "./templates.js";
...
console.log("\n7. Template routing discriminates and refuses when unsure");
{
  ...
  // Every shipped template is built, so routing to a non-freight one is now a positive
  // check: the registry has to discriminate between eleven vocabularies, not just find one.
  const recruit = pick(spec({ request: "track a candidate through interview to placement" }));
  ok("routes recruitment, not freight", recruit.template?.id === "recruitment", String(recruit.template?.id));

  const vet = pick(spec({ request: "book the pet in for a vaccination and discharge the animal" }));
  ok("routes veterinary", vet.template?.id === "veterinary", String(vet.template?.id));

  const court = pick(spec({ request: "run a conflict check before the hearing and bill the matter" }));
  ok("routes legal", court.template?.id === "legal", String(court.template?.id));

  // The ready:false refusal is still the behaviour that matters, so it is exercised
  // against an injected registry rather than by keeping a broken template in production.
  const unbuilt = pick(
    spec({ request: "track a candidate through interview to placement" }),
    0.08,
    [{ ...TEMPLATES.find((t) => t.id === "recruitment")!, ready: false }],
  );
  ok("will not route to an unbuilt template", unbuilt.template === null);
  ok("but says which one matched", unbuilt.matchedButNotReady.includes("recruitment"));
